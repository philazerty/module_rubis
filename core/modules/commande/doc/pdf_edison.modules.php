 ...
 *	\ingroup    commande
 *	\brief      File of Class to generate PDF orders with template edison
 */
...
/**
 *	Class to generate PDF orders with template edison
 */
class pdf_edison extends ModelePDFCommandes
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$outputlangs->loadLangs(array("main", "dict", "companies", "bills", "products", "orders", "deliveries", "rubis@rubis"));
		...
		if ($conf->commande->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				if (!empty($conf->global->RUBIS_LINE_COLOR_EDISON)){
					$linecolor=explode(",",$conf->global->RUBIS_LINE_COLOR_EDISON);
				...
				*/
				...
				if (!empty($conf->global->RUBIS_SIGNATURE_AREA_EDISON)){
					$posy+=5;
					$cury=-59;
					$deltax=-85;
						// Update by Philazerty
	...
	protected function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		*/
		...
		{
			$pdf->SetXY($this->marge_gauche, $posy);
			$depositcolor=explode(",",$conf->global->RUBIS_DEPOSIT_COLOR_EDISON);  //Si la constante n'existe pas, tout sera à zéro donc en noir
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey = "PdfOrderTitle")
	{
		...
		$outputlangs->loadLangs(array("main", "bills", "propal", "orders", "companies", "rubis@rubis"));
		...
		if (empty($conf->global->PDF_DISABLE_MYCOMPANY_LOGO))
		{
			if ($this->emetteur->logo)
			{
				...
				if (empty($conf->global->MAIN_PDF_USE_LARGE_LOGO))
				{
					...
					if (!empty($conf->global->RUBIS_LOGO_EDISON)){
						$logo=$logodir.'/logos/'.$conf->global->RUBIS_LOGO_EDISON;
					}else{
