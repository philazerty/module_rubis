...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_homard.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from homard model
 */
...
/**
 *	Class to generate the customer invoice PDF with template homard
 */
class pdf_homard extends ModelePDFFactures
{
     /**
	...
	public function __construct($db)
	{
		...
		//Add by Philazerty for Rubis
		$langs->load("rubis@rubis");
		
		$this->db = $db;
		$this->name = "homard";
		$this->description = $langs->trans('PDFhomardDescription');
    ...
    public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		//Update by Philazerty for Rubis
		// Load translation files required by the page
		$outputlangs->loadLangs(array("main", "bills", "products", "dict", "companies", "rubis@rubis"));
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$pdf->SetCreator("Dolibarr ".DOL_VERSION." + ".$outputlangs->transnoentities("RubisVersion")." (".$this->name.")");
				...
				if ($conf->incoterm->enabled)
				{
					...
					if ($desc_incoterms)
					{
						...
						// Update by Philazerty
						if (!empty($conf->global->RUBIS_LINE_COLOR_HOMARD)){
							$linecolor=explode(",",$conf->global->RUBIS_LINE_COLOR_HOMARD);
				...
				// Add by Philazerty
				// Ajout des informations Chorus
				include DOL_DOCUMENT_ROOT.'/custom/rubis/includes/rubis_add_chorus.inc.php';
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					if (!empty($conf->global->MAIN_PDF_DASH_BETWEEN_LINES) && $i < ($nblines - 1))
					{
						...
						if (!empty($conf->global->RUBIS_LINE_COLOR_HOMARD)){
							$linecolor=explode(",",$conf->global->RUBIS_LINE_COLOR_HOMARD);
							//$pdf->SetDrawColor($linecolor[0],$linecolor[1],$linecolor[2]);
							$pdf->SetLineStyle(array('dash'=>'1,1', 'color'=>$linecolor));
						}else{
				...
				// Add by Philazerty
				// Avertissement sur les pdf
				include DOL_DOCUMENT_ROOT.'/custom/rubis/includes/rubis_add_warning_on_pdf.inc.php';

				// Pagefoot
				$this->_pagefoot($pdf, $object, $outputlangs);
				
				// Add by Philazerty
				// Ajout des CGV dans le document - Add Conditions and Terms in documents
				include DOL_DOCUMENT_ROOT.'/custom/rubis/includes/rubis_add_cgv.inc.php';
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$outputlangs->loadLangs(array("main", "bills", "propal", "companies", "rubis@rubis"));
		...
		// Add by Philazerty
		// Ajout d'un repère de pliage / Add a fold mark
		include DOL_DOCUMENT_ROOT.'/custom/rubis/core/includes/rubis_fold_mark.inc.php';
		...
		if (empty($conf->global->PDF_DISABLE_MYCOMPANY_LOGO))
		{
			if ($this->emetteur->logo)
			{
				...
				if (empty($conf->global->MAIN_PDF_USE_LARGE_LOGO))
				{
					...
					// Logo spécifique RUBIS / Rubis Specific logo
					// Update by Philazerty 13/11/2017
					if (!empty($conf->global->RUBIS_LOGO_HOMARD)){
						$logo=$logodir.'/logos/'.$conf->global->RUBIS_LOGO_HOMARD;
					}else{
	...
	function _pagelcr(&$pdf, $object, $posy, $outputlangs,$hidefreetext=0)
	{
		...
		$cury+=3;
		$pdf->SetXY(90, $cury);
		$pdf->Cell(100, 3, $outputlangs->transnoentities('HomardDocLCR1'), 0, 1, 'L', 0);
		$cury+=3;
		$pdf->SetXY(90, $cury);
		$pdf->Cell(100, 3, $outputlangs->transnoentities('HomardDocLCR2'), 0, 1, 'L', 0);
		$cury+=3;
		$pdf->SetXY(90, $cury);
		$pdf->Cell(100, 3, $outputlangs->transnoentities('HomardDocLCR3'), 0, 1, 'L', 0);
		$cury+=3;
		$pdf->SetXY(90, $cury);
		$pdf->Cell(100, 3, $outputlangs->transnoentities('HomardDocLCR4'), 0, 0, 'L', 0);

		// Sender properties
		$pdf->SetFont('helvetica','B',8);
		$pdf->SetXY(130, $cury-9);
		...
		$pdf->SetXY(180, $cury);
		$pdf->SetFont('helvetica','',7);
		$pdf->Cell(18, 0, $outputlangs->transnoentities('HomardDocLCR5'),0,1,C);
		...
		$pdf->SetFont('helvetica','',7);
		$pdf->Cell($largeur_cadre, 0, $outputlangs->transnoentities('HomardDocLCR6'),0,0,C);
		...
		$pdf->SetXY($curx, $cury-1);
		$pdf->SetFont('helvetica','',7);
		$pdf->Cell($largeur_cadre, 0, $outputlangs->transnoentities('HomardDocLCR7'),0,0,C);
		$pdf->Line($curx, $cury, $curx, $cury+$hauteur_cadre);
		$pdf->Line($curx, $cury+$hauteur_cadre, $curx+$largeur_cadre, $cury+$hauteur_cadre);
		$pdf->Line($curx+$largeur_cadre, $cury, $curx+$largeur_cadre, $cury+$hauteur_cadre);
		$pdf->SetXY($curx, $cury+2.5);
		...
		$pdf->SetXY($curx, $cury-1);
		$pdf->SetFont('helvetica','',7);
		$pdf->Cell($largeur_cadre, 0, $outputlangs->transnoentities('HomardDocLCR8'),0,0,C);
		$pdf->Line($curx, $cury, $curx, $cury+$hauteur_cadre);
		$pdf->Line($curx, $cury+$hauteur_cadre, $curx+$largeur_cadre, $cury+$hauteur_cadre);
		$pdf->Line($curx+$largeur_cadre, $cury, $curx+$largeur_cadre, $cury+$hauteur_cadre);
		$pdf->SetXY($curx, $cury+2.5);
		...
		$pdf->SetXY($curx, $cury-1);
		$pdf->SetFont('helvetica','',7);
		$pdf->Cell($largeur_cadre, 0, $outputlangs->transnoentities('HomardDocLCR9'),0,0,C);
		...
		$pdf->SetXY($curx, $cury-1);
		$pdf->SetFont('helvetica','',7);
		$pdf->Cell($largeur_cadre, 0, $outputlangs->transnoentities('HomardDocLCR10'),0,0,C);
		$pdf->Line($curx, $cury, $curx, $cury+$hauteur_cadre);
		$pdf->Line($curx, $cury+$hauteur_cadre, $curx+$largeur_cadre, $cury+$hauteur_cadre);
		$pdf->Line($curx+$largeur_cadre, $cury, $curx+$largeur_cadre, $cury+$hauteur_cadre);
		$pdf->SetXY($curx, $cury+2.5);
		...
		$pdf->SetXY($curx, $cury);
		$pdf->SetFont('helvetica','B',8);
		$pdf->Cell($largeur_cadre, 1, $outputlangs->convToOutputCharset($object->thirdparty->code_client),0,0,C);
		...
		$largeur_cadre=70;
		$hauteur_cadre=6;
		$sql = "SELECT rib.fk_soc, rib.domiciliation, rib.code_banque, rib.code_guichet, rib.number, rib.cle_rib, rib.iban_prefix, rib.bic";
		...
		if ($resql)
		{
			...
			while ($i <= $num)
			{
				...
				$pdf->Cell($largeur_cadre, 1, $outputlangs->transnoentities('HomardDocLCR11').'    '.    $outputlangs->transnoentities('HomardDocLCR12').'        '.          $outputlangs->transnoentities('HomardDocLCR13').'            '.            $outputlangs->transnoentities('HomardDocLCR14'),0,0,L);
				$curx=150;
				$largeur_cadre=55;
				$pdf->SetXY($curx, $cury-1);
				$pdf->SetFont('helvetica','',6);
				$pdf->Cell($largeur_cadre, 1, $outputlangs->transnoentities('HomardDocLCR15'),0,0,C);
				$pdf->SetXY($curx, $cury+2);
				$pdf->SetFont('helvetica','B',8);
		...
		$pdf->SetXY($curx, $cury);
		$pdf->SetFont('helvetica','',6);
		$pdf->Cell($largeur_cadre, 1, $outputlangs->transnoentities('HomardDocLCR16'),0,0,L);
		// Row
		...
		$pdf->SetXY($curx, $cury);
		$pdf->SetFont('helvetica','',6);
		$pdf->MultiCell($largeur_cadre, $hauteur_cadre, "Nom \n et Adresse \n".$outputlangs->transnoentities('HomardDocLCR17'),0,R);
		...
		$pdf->SetFont('helvetica','',6);
		$pdf->MultiCell($largeur_cadre, 4, $outputlangs->transnoentities('HomardDocLCR18'),0,R);
		...
		$pdf->SetFont('helvetica','',6);
		$pdf->MultiCell($largeur_cadre*2, 4, $outputlangs->transnoentities('HomardDocLCR19'),0,C);

		$pdf->Line(0,$this->page_hauteur-$this->marge_basse,$this->page_largeur, $this->page_hauteur-$this->marge_basse);
		//$pdf->Line(0,110,$this->page_largeur, 110);
		$pdf->SetXY($this->page_largeur-65,$this->page_hauteur-$this->marge_basse-3 );
		$pdf->SetFont('helvetica','',6);
		$pdf->MultiCell(50, 4, $outputlangs->transnoentities('HomardDocLCR20'),0,R);
	}
}
