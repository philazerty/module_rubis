 ...
 * Copyright (C) 2017	   Philippe SAGOT (Philazerty)  <courrier@mon-dolibarr.fr>
 *
...
/**
 *	Class to manage PDF invoice template homard
 */
class pdf_homard extends ModelePDFFactures
{
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$outputlangs->load("companies");
		//Add by Philazerty for Rubis
		$outputlangs->load("pack_rubis@rubis");

		$default_font_size = pdf_getPDFFontSize($outputlangs);

		pdf_pagehead($pdf,$outputlangs,$this->page_hauteur);
		
		// Ajout repère de pliage à 1/3 de la hauteur 
		// Update by Philazerty 13/11/2017
		if (!empty($conf->global->RUBIS_FOLD_MARK))
		{
			$pdf->Line(0,($this->page_hauteur)/3,3,($this->page_hauteur)/3);
			//$pdf->Line(0,($this->page_hauteur)/3*2,3,($this->page_hauteur)/3*2);
		}
		...
		// Logo spécifique HOMARD / HOMARD Specific logo
		// Update by Philazerty 13/11/2017
		if (!empty($conf->global->RUBIS_LOGO_HOMARD)){
			$logo=$conf->mycompany->dir_output.'/logos/'.$conf->global->RUBIS_LOGO_HOMARD;
		}else{
