 ...
 * Copyright (C) 2017-2023 Philazerty  			<dolibarr@philazerty.fr>
 *
...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_rubis.modules.php
 *	\ingroup    propale
 *	\brief      File of Class to generate PDF proposal with rubis template
 */
...
/**
 *	Class to generate PDF proposal rubis
 */
class pdf_rubis extends ModelePDFPropales
{
	/**
	...
	public function __construct($db)
	{
		...
		// Translations
		$langs->loadLangs(array("main", "bills", "rubis@rubis"));
		...
		$this->db = $db;
		$this->name = "rubis";
		$this->description = $langs->trans('PDFrubisDescription');
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		//Update by Philazerty for Rubis
		$outputlangs->loadLangs(array("main", "dict", "companies", "bills", "propal", "products", "rubis@rubis"));
		...
		if ($conf->propal->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				if (!empty($conf->global->RUBIS_LINE_COLOR)){
					$linecolor=explode(",",$conf->global->RUBIS_LINE_COLOR);
				...
				// Update by Philazerty
				$pdf->SetCreator("Dolibarr ".DOL_VERSION." + ".$outputlangs->transnoentities("RubisVersion"));
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if (!empty($conf->global->MAIN_PDF_DASH_BETWEEN_LINES) && $i < ($nblines - 1)) {
						...
						// Update by Philazerty
						// Change dividing line color in frame
						if (!empty($conf->global->RUBIS_LINE_COLOR)){
							$linecolor=explode(",",$conf->global->RUBIS_LINE_COLOR);
							//$pdf->SetDrawColor($linecolor[0],$linecolor[1],$linecolor[2]);
				...
				*/
				...
				// Update by philazerty
				if (empty($conf->global->RUBIS_SIGNATURE_AREA)){   // Si la signature de Rubis n'est pas activée
				...
				// Add by Philazerty
				// Ajout zone de signature RUBIS
				if (!empty($conf->global->RUBIS_SIGNATURE_AREA)){
					$posy+=5;
					$cury=-59;
					$deltax=-85;
					if (!empty($conf->global->RUBIS_SIGNATURE_AREA2)){
						$pdf->SetXY($deltax - 5, $cury-2);
						$pdf->SetFont('','', $default_font_size - 3);
						$pdf->MultiCell(80, 3, $outputlangs->transnoentities("RubisReturnSigned3"),0,'L',0);
						$pdf->SetXY($deltax - 5, $cury + 1);
						$pdf->SetFont('','', $default_font_size - 3);
						$pdf->MultiCell(80, 3, $outputlangs->transnoentities("RubisReturnSigned4"),0,'L',0);
						// Update by Philazerty
						if (!empty($conf->global->RUBIS_LINE_COLOR)){
							$linecolor=explode(",",$conf->global->RUBIS_LINE_COLOR);
							$pdf->SetDrawColor($linecolor[0],$linecolor[1],$linecolor[2]);
						...
						$pdf->RoundedRect(120,242,80, 8,2); 
						$pdf->SetXY($deltax - 5, $cury+12);
						$pdf->SetFont('','', $default_font_size - 3);
						$pdf->MultiCell(80, 3, $outputlangs->transnoentities("RubisSignatureStamp"),0,'L',0);
						// Update by Philazerty
						if (!empty($conf->global->RUBIS_LINE_COLOR)){
							$linecolor=explode(",",$conf->global->RUBIS_LINE_COLOR);
							$pdf->SetDrawColor($linecolor[0],$linecolor[1],$linecolor[2]);
						...
						$pdf->SetLineWidth(0.2);
						$pdf->RoundedRect(120,253,80, 18,2); 
						$pdf->SetXY($deltax - 5, $cury+17);
						$pdf->SetFont('','', $default_font_size - 3);
						$pdf->MultiCell(80, 3, $outputlangs->transnoentities("RubisDateSigned"),0,'L',0);
					}else{
						// Update by Philazerty
						if (!empty($conf->global->RUBIS_LINE_COLOR)){
							$linecolor=explode(",",$conf->global->RUBIS_LINE_COLOR);
						...
						$pdf->MultiCell(80, 5, $outputlangs->transnoentities("RubisGoodForAgreement"), 0, 'L');
						$pdf->SetFont('','', $default_font_size - 1);
						$pdf->SetXY($deltax - 0, $cury+10);
						$pdf->SetFont('','', $default_font_size - 3);
						$pdf->MultiCell(80, 3, $outputlangs->transnoentities("RubisReturnSigned1"),0,'L',0);
						$pdf->SetXY($deltax - 0, $cury+13);
						$pdf->MultiCell(80, 3, $outputlangs->transnoentities("RubisReturnSigned2"),0,'L',0);
						$pdf->SetXY($deltax - 0, $cury+18);
						$pdf->SetFont('','', $default_font_size - 3);
						$pdf->MultiCell(80, 3, $outputlangs->transnoentities("RubisDateSigned"), 0, 'L', 0);
						$pdf->SetXY($deltax - 0, $cury+23);
						$pdf->MultiCell(80, 3, $outputlangs->transnoentities("RubisPlaceSigned"), 0, 'L', 0);
	...
	protected function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		// Ajout du montant de l'acompte / Add deposit amount
		
		if (!empty($object->deposit_percent)){
			$rubisdepositpercent=$object->deposit_percent; // Dans conditions de paiement
		}else{
			$rubisdepositpercent = $object->array_options['options_deposit'];
		}
		if ($rubisdepositpercent>0)
		{
			$pdf->SetXY($this->marge_gauche, $posy);
			$depositcolor=explode(",",$conf->global->RUBIS_DEPOSIT_COLOR);  //Si la constante n'existe pas, tout sera à zéro donc en noir
			$pdf->SetTextColor($depositcolor[0],$depositcolor[1],$depositcolor[2]);
			$pdf->SetFont('','B', $default_font_size - 2);
			$deposit = round( ($rubisdepositpercent/100) * $object->total_ttc,$conf->global->RUBIS_DEPOSIT_ROUNDING);
			$pdf->MultiCell(90, 3, $outputlangs->transnoentities("RubisPaymentDepositValue").price($deposit, 2, $outputlangs).$outputlangs->transnoentities("RubisPaymentDepositValue2"), 0, 'L', 0);
			$pdf->SetTextColor(0,0,0);
			$posy=$pdf->GetY()+1;
		}
		// Fin RUBIS Deposit 
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		// Update by Philazerty
		// Changement de la couleur du tableau / Change color tab
		if (!empty($conf->global->RUBIS_LINE_COLOR)){
			$linecolor=explode(",",$conf->global->RUBIS_LINE_COLOR);
			$pdf->SetDrawColor($linecolor[0],$linecolor[1],$linecolor[2]);
		}else{
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		// Add by Philazerty
		// Ajout du repère de pliage / Add the fold mark
		include DOL_DOCUMENT_ROOT.'/custom/rubis/includes/rubis_fold_mark.inc.php';
		...
		if (empty($conf->global->PDF_DISABLE_MYCOMPANY_LOGO)) {
			if ($this->emetteur->logo) {
				...
				if (empty($conf->global->MAIN_PDF_USE_LARGE_LOGO)) {
					...
					if (!empty($conf->global->RUBIS_LOGO_RUBIS)){
						$logo=$logodir.'/logos/'.$conf->global->RUBIS_LOGO_RUBIS;
					}else{
		...
		*/
		...
		if ($showaddress) {
			...
			if (empty($conf->global->MAIN_PDF_NO_RECIPENT_FRAME)) {
				...
				// Update by Philazerty
				// Changement couleur cadre adresse / Change adress box color
				if (!empty($conf->global->RUBIS_LINE_COLOR)){
					$linecolor=explode(",",$conf->global->RUBIS_LINE_COLOR);
					$pdf->SetDrawColor($linecolor[0],$linecolor[1],$linecolor[2]);
				}else{
