...
$altotemplate=$extrafields->showOutputField("altotemplate", $object->array_options["options_altotemplate"]);

// For debug
//$pdf->writeHTMLCell(190, 3, $this->posxdesc-1, $tab_top-1, dol_htmlentitiesbr($altotemplate), 0, 1);

if ($conf->global->MAIN_MULTILANGS) {
  $altotemplate_pdf=DOL_DATA_ROOT."/doctemplates/contracts/".$outputlangs->defaultlang."/".$altotemplate.".pdf";
}else{
  $altotemplate_pdf=DOL_DATA_ROOT."/doctemplates/contracts/".$altotemplate.".pdf";
}

if (file_exists($altotemplate_pdf)){
            $pagecount = $pdf->setSourceFile($altotemplate_pdf);
...
//	$warningmsg=$outputlangs->transnoentities("AltoTemplateNotFound");
//	setEventMessages($warningmsg, null, 'warnings');
	dol_syslog('alto template not found at '.$altotemplate_pdf, LOG_DEBUG);
}
?>
